...
class StructureAndSystemsContainer extends Component {
  ...
  componentDidMount() {
    ...
    fetchQuestions({ questionType: 'structure_and_systems' });
  }

  onFinish = data => {
    console.log('🚀 ~ file: index.js ~ line 62 ~ StructureAndSystemsContainer ~ data', data);
  }

  sendDataToServer = (sender) => {
    const resultAsString = JSON.stringify(sender.data);
    console.log('🚀 ~ file: index.js ~ line 50 ~ StructureAndSystemsContainer ~ resultAsString', resultAsString);
  }
  ...
  render() {
    return (
      <div className="structure-and-systems-container">
        <PageHeader
